...
import { PUBLIC_ALL_THEME, PUBLIC_GET_THEME_DETAIL_V2 } from '@api/constant';
...
export async function getServerSideProps({ req, res }: NextPageContext) {
  ...
  const dataTheme: any = await (
    await fetch(`${PREFIX_API_IP_PIST}${PUBLIC_ALL_THEME}`)
  ).json();

  for await (const item of dataTheme?.data) {
    const data: any = await (
      await fetch(`${PREFIX_API_IP_COMMUNITY}${PUBLIC_GET_THEME_DETAIL_V2(item?.code)}`)
    ).json();
